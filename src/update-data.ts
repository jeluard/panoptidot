...
function extractAssociatedEvents(
  ...
  return allEvents
    ...
    .map(({ data, section, method }) => {
      return { section, method, data: data.map((o) => o.toJSON()) };
    });
}
...
function extractExtrinsicFailedEvent(
  events: Array<Event>
): DispatchError | undefined {
  // const decoded = api.registry.findMetaError(dispatchError.asModule);
  return extractEvent(events, 'system', 'ExtrinsicFailed')
...
type Delegatees = Record<Address, Record<TrackId, Delegation>>;

type Delegates = Record<Address, Delegatees>;
...
function createIndexes(
  allData: Record<number, Record<string, Record<string, Extrinsic>>>
): { delegates: Delegates } {
...
async function extractAll(
  ...
  for (const [rangeFrom, rangeTo] of ranges(from, to, 10_000)) {
    ...
    if (!(await checkFileExists(fileName))) {
      ...
      const newData = await extractAllExtrinsics(
        api,
        rangeFrom,
        rangeTo,
        structure
